...
const TaskInput = ({ taskInputValue, setTaskInputValue, taskDescInputValue, setTaskDescInputValue, addTask }) => {
  return (
    <div className='task-box w-[70%] p-[10px] rounded-lg flex flex-col items-center gap-6 border-1 mx-auto'>
...
const TaskItem = ({ element, index, removeTask }) => {
  return (
    <Fragment key={index}>
      <div className='mx-auto my-2 flex flex-col md:flex-row gap-2 w-[70%] bg-blue-500 text-lg text-white p-2 rounded-lg hover:border-1 dark:border-white border-black hover:shadow-[0px_3px_6px_rgba(0,0,0,0.16),0px_3px_6px_rgba(0,0,0,0.23)] transition duration-300'>
...
const FooterPart = () => {
  return (
    <footer className=' w-full h-fit grid grid-cols-1 xl:grid-cols-3 bg-blue-400 dark:bg-blue-700 dark:text-white p-5 mt-10'>
...
const App = () => {
  ...
  return (
    <div className='min-h-screen flex flex-col items-center gap-2 dark:bg-black dark:text-white'>
      <NavBar dark={dark} setDark={setDark} />
      <div className='min-h-[70vh] w-[100%] mx-auto overflow-auto'>
        <TaskInput 
